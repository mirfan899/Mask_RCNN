...
class InvoiceDataset(utils.Dataset):
    ...
    def load_invoice(self, dataset_dir, subset):
        """Load a subset of the Balloon dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes. We have only one class to add.
        self.add_class('object', 1, 'Address')
        self.add_class('object', 2, 'Name')
        self.add_class('object', 3, 'Table')
        self.add_class('object', 4, 'Date')
        self.add_class("object", 5, "Email")
        self.add_class("object", 6, "Website")
        ...
        for a in annotations:
            ...
            name_dict = {"Address": 1, "Name": 2, "Table": 3, "Date": 4, "Email": 5, "Website": 6}
            # key = tuple(name_dict)
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        for i, p in enumerate(info["rectangles"]):
            ...
            rr, cc = skimage.draw.polygon([p['y'], p['width']], [p["x"], p["height"]])
            mask[rr, cc, i] = 1
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = InvoiceDataset()
    dataset_train.load_invoice(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = InvoiceDataset()
    dataset_val.load_invoice(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect Rectangles.')
